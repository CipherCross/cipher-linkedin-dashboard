...
| --- | --- | --- |
| `phase0-drawer-export` | `src/ui/Overlay.tsx` implements `Drawer` and `src/ui/index.ts` exports it; current route consumer count is zero. `ConversationDrawer` is a separate product component and does consume modal behavior. | Phase 1 deletes the unused primitive after a fresh zero-consumer scan. |
| `phase0-refreshing-region-export` | `src/ui/States.tsx` implements `RefreshingRegion` and `src/ui/index.ts` exports it; current consumer count is zero. Routes currently use `UpdatingNote`/local state directly. | Phase 1 deletes this unused primitive; UpdatingNote plus aria-busy is the accepted refresh contract. |
