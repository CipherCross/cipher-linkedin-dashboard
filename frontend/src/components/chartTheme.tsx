...
/** Every SVG text size in a chart: axis ticks, reference-line labels, scatter
 *  labels. Recharts writes these as SVG attributes, which cannot read a CSS
 *  var(), so this mirrors --text-meta (13px, the metadata floor) as a number.
 *  Use it for every `fontSize` a chart prop takes — never a bare literal. */
export const CHART_TEXT_SIZE = 13
...
export const AXIS = {
  ...
  tick: { fill: 'var(--text-muted)', fontSize: CHART_TEXT_SIZE },
} as const
...
export const TOOLTIP = {
  ...
  labelStyle: { color: 'var(--text)', fontWeight: 600, marginBottom: 'var(--space-xs)' },
...
export const legendText = (value: string) => (
  <span className="text-app-text-secondary text-app-meta">{value}</span>
)
...
export function ChartEmpty({ height = 240, label }: { height?: number; label: string }) {
  return (
    <div className="flex items-center justify-center mt-app-xs text-app-text-muted text-app-meta bg-app-surface-2 border border-dashed border-app-border rounded-md" style={{ height }}>
      <span>{label}</span>
    </div>
  )
}
