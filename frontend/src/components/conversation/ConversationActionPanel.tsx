...
export function ConversationActionPanel({ workflow, members = [], inboundRevision = 0, persistedDoNotContact, saving, error, onSave, onDraftChange, onDirtyChange, externalActions = false, onValidityChange }: ConversationActionPanelProps) {
  ...
  return <section className="replies-action-panel" aria-label="Дальнейшее действие">
    <div className="replies-panel-heading"><div><h2>Следующий шаг</h2></div>{workflow?.acknowledged_inbound_revision !== inboundRevision && inboundRevision > 0 && <span className="replies-pending-badge">Нужен шаг</span>}</div>
    ...
    <label className="replies-fieldset"><span className="replies-label"><UserRound size={14} /> Ответственный за диалог</span><select value={owner == null ? '' : String(owner)} onChange={(event) => { const next = event.target.value ? Number(event.target.value) : null; setOwner(next); updateDraft({ owner_id: next }) }}><option value="">Не назначен</option>{members.filter((member) => member.active).map((member) => <option key={member.id} value={member.id}>{member.name}{members.filter((candidate) => candidate.active && candidate.name === member.name).length > 1 ? ` · #${member.id}` : ''}</option>)}</select></label>
    ...
    {(removingDnc || reason) && <label className="replies-fieldset"><span className="replies-label">Комментарий изменения {removingDnc && <span className="muted small">обязателен при снятии запрета</span>}</span><input value={reason} onChange={(event) => { const next = event.target.value; setReason(next); updateDraft({ change_reason: next.trim() || null }) }} placeholder="Например, контакт снова разрешил связь…" /></label>}
    {!removingDnc && !reason && <button type="button" className="replies-collapse" onClick={() => setReason(' ')}>Добавить комментарий</button>}
    {error && <div className="replies-inline-error" role="alert">{error}</div>}
    {!externalActions && <button className="secondary replies-save" type="button" onClick={submit} disabled={saving || invalid}>{saving ? 'Сохраняем…' : <><Check size={16} /> Сохранить действие</>}</button>}
  </section>
}
