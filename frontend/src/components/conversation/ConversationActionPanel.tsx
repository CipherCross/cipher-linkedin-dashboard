...
export function ConversationActionPanel({ workflow, members = [], inboundRevision = 0, persistedDoNotContact, saving, error, onSave, onDraftChange, onDirtyChange, externalActions = false, onValidityChange }: ConversationActionPanelProps) {
  ...
  return <section className="replies-action-panel" aria-label="Next step">
    ...
    <SelectField
      ...
      value={action ?? ''}
      disabled={dnc}
      onChange={(event) => { const next = (event.target.value || null) as ReplyWorkflowAction | null; setAction(next); updateDraft({ action: next }) }}
    >
      <option value="">Not set</option>
      {(Object.keys(ACTION_LABELS) as ReplyWorkflowAction[]).map((value) => <option key={value} value={value}>{ACTION_LABELS[value]}</option>)}
    </SelectField>
    <SelectField
      label={<span className="flex items-center gap-app-xs"><UserRound size={14} aria-hidden="true" /> Conversation owner</span>}
      value={owner == null ? '' : String(owner)}
      onChange={(event) => { const next = event.target.value ? Number(event.target.value) : null; setOwner(next); updateDraft({ owner_id: next }) }}
    >
      <option value="">Unassigned</option>
      {members.filter((member) => member.active).map((member) => <option key={member.id} value={member.id}>{member.name}{members.filter((candidate) => candidate.active && candidate.name === member.name).length > 1 ? ` · #${member.id}` : ''}</option>)}
    </SelectField>
    {requiresDate && <TextField
      label="Follow-up date"
      type="date"
      value={date}
      min={madridToday}
      onChange={(event) => { setDate(event.target.value); updateDraft({ next_follow_up_date: event.target.value || null }) }}
    />}
    <Checkbox
      label="Do not contact"
      hint="Setting this cancels follow-ups inside the dashboard. Stop the Linked Helper campaign separately."
      checked={dnc}
      onChange={(event) => { const next = event.target.checked; setDnc(next); setAction(next ? 'resolved' : null); updateDraft({ do_not_contact: next, action: next ? 'resolved' : null }) }}
    />
    ...
    {(removingDnc || reason) && <TextField
      label={<>Change comment {removingDnc && <span className="text-app-text-muted text-app-meta">required when removing Do not contact</span>}</>}
      value={reason}
      onChange={(event) => { const next = event.target.value; setReason(next); updateDraft({ change_reason: next.trim() || null }) }}
      placeholder="For example: the contact agreed to be contacted again…"
    />}
