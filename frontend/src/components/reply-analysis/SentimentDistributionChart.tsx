...
export function DistributionChart({
  ...
  return (
    <div className="sa-bars" role="list" aria-label="Sentiment distribution">
      {entries.map(([key, metric]) => (
        (() => { const href = linkFor(metric, key); const content = <><span className="sa-bar-label">{labels[key] ?? key}</span><span className="sa-bar-track" aria-hidden="true"><span className="sa-bar-fill" style={{ width: `${Math.round(metric.numerator / max * 100)}%`, background: COLORS[key] ?? 'var(--accent)' }} /></span><span className="sa-bar-value">{metric.numerator.toLocaleString(UI_LOCALE)} <small>{metric.rate == null ? '—' : `${(metric.rate * 100).toFixed(1)}%`}</small></span></>; return href ? <Link key={key} to={href} className="sa-bar-row" role="listitem">{content}</Link> : <div key={key} className="sa-bar-row sa-disabled-drilldown" role="listitem">{content}</div> })()
      ))}
    </div>
  )
}
