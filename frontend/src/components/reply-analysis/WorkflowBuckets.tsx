...
export function WorkflowBuckets({ rows, linkFor, showZero = true }: { rows: Readonly<Record<string, AnalyticsMetric>>; linkFor: (metric: AnalyticsMetric, key: string) => string | null; showZero?: boolean }) {
  ...
  return (
    <div className="grid gap-group grid-cols-[repeat(auto-fit,minmax(260px,1fr))]" role="list" aria-label="Current state of work">
      {keys.length === 0 && <p className="text-app-text-muted">No open next steps.</p>}
      {keys.map((key) => {
        const metric = rows[key] ?? { numerator: 0, denominator: 0, rate: null }
        const href = linkFor(metric, key); const content = <><span>{WORKFLOW_LABELS[key] ?? key}</span><strong>{metric.numerator.toLocaleString(UI_LOCALE)}</strong><small>{metric.rate == null ? '—' : `${(metric.rate * 100).toFixed(1)}%`}</small></>; return href ? <Link to={href} className="grid items-center gap-x-app-md gap-y-0.5 grid-cols-[1fr_auto] min-h-row px-pane py-app-sm border border-app-border rounded-control bg-app-surface text-app-text no-underline [a&:hover]:border-app-accent [a&:hover]:bg-app-surface-2 [&_span]:min-w-0 [&_span]:text-app-table [&_strong]:text-app-section [&_strong]:tabular-nums [&_small]:col-span-full [&_small]:text-app-text-muted [&_small]:text-app-meta" key={key} role="listitem">{content}</Link> : <div className={`${SA_DISABLED} grid items-center gap-x-app-md gap-y-0.5 grid-cols-[1fr_auto] min-h-row px-pane py-app-sm border border-app-border rounded-control bg-app-surface text-app-text no-underline [a&:hover]:border-app-accent [a&:hover]:bg-app-surface-2 [&_span]:min-w-0 [&_span]:text-app-table [&_strong]:text-app-section [&_strong]:tabular-nums [&_small]:col-span-full [&_small]:text-app-text-muted [&_small]:text-app-meta`} key={key} role="listitem">{content}</div>
      })}
      {(rows.transfers || rows.transfer) && (showZero || (rows.transfers ?? rows.transfer)!.numerator > 0) && (() => { const transfer = rows.transfers ?? rows.transfer!; const href = linkFor(transfer, 'transfers'); const content = <><span>Handovers (events)</span><strong>{transfer.numerator.toLocaleString(UI_LOCALE)}</strong><small>counted separately from statuses</small></>; return href ? <Link to={href} className="grid items-center gap-x-app-md gap-y-0.5 grid-cols-[1fr_auto] min-h-row px-pane py-app-sm border border-app-border rounded-control bg-app-surface text-app-text no-underline [a&:hover]:border-app-accent [a&:hover]:bg-app-surface-2 [&_span]:min-w-0 [&_span]:text-app-table [&_strong]:text-app-section [&_strong]:tabular-nums [&_small]:col-span-full [&_small]:text-app-text-muted [&_small]:text-app-meta border-dashed" role="listitem">{content}</Link> : <div className={`${SA_DISABLED} grid items-center gap-x-app-md gap-y-0.5 grid-cols-[1fr_auto] min-h-row px-pane py-app-sm border border-app-border rounded-control bg-app-surface text-app-text no-underline [a&:hover]:border-app-accent [a&:hover]:bg-app-surface-2 [&_span]:min-w-0 [&_span]:text-app-table [&_strong]:text-app-section [&_strong]:tabular-nums [&_small]:col-span-full [&_small]:text-app-text-muted [&_small]:text-app-meta border-dashed`} role="listitem">{content}</div> })()}
    </div>
  )
}
