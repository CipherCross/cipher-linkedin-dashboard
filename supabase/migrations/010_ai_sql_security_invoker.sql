-- Postgres prohibits SET ROLE inside SECURITY DEFINER functions ("cannot set
-- parameter \"role\" within security-restricted operation"). Switch to
-- SECURITY INVOKER and make service_role (the RPC caller) a member of
-- ai_readonly so the role drop is permitted. EXECUTE stays revoked from
-- anon/authenticated, so only the service-role key can call this.

grant ai_readonly to service_role;
