...
for (const role of ["app_owner", "app_runtime", "app_readonly"]) {
  const roleBlock = artifact.match(new RegExp(`CREATE ROLE ${role}[\\s\\S]*?;`))?.[0] ?? "";
  assert(roleBlock.includes("NOLOGIN"), `${role} must be NOLOGIN`);
