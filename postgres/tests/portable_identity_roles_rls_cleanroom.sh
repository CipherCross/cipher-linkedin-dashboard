...
docker exec --interactive "$container" psql --username postgres --dbname postgres --set ON_ERROR_STOP=1 < "$identity_artifact" >/dev/null
docker exec --interactive "$container" psql --username postgres --dbname postgres --set ON_ERROR_STOP=1 < "$catalog_assertions"
docker exec --interactive "$container" psql --username postgres --dbname postgres --set ON_ERROR_STOP=1 < "$behavior_assertions"
echo "Portable identity, roles and RLS clean-room apply passed using $image"
